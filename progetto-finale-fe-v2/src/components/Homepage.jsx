...
function Homepage() {
      ...
      try {
        ...
        if (asteResponse.data.length > 0) {
          ...
          for (const asta of asteResponse.data) {
            try {
              const itemResponse = await axios.get(`http://localhost:8080/api/items/${asta.itemId}`, {
                headers: { Authorization: sessionId }
              });
  ...
  return (
  <div className="min-h-screen bg-gradient-to-br from-white to-gray-100">      {/* Header */}
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div 
                ...
                <span className="block text-xs text-gray-600">{user?.ruolo}</span>
             </div>
                <button
      ...
      <main className="max-w-[90%] mx-auto px-8 py-8">
        {error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {asteAttive.map((asta) => (
              <div key={asta.id} className="bg-white rounded-lg shadow-md p-6 flex flex-col min-h-[400px] 
                ...
                <div className="text-center mb-4">
                  ...
                  {user?.ruolo === "GESTORE" && (
                    <div className="text-sm text-gray-500 mt-1">
                      <p>ID Asta: {asta.id} • ID Item: {asta.itemId}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="border-t border-b border-gray-100 py-4">
                  ...
                  <div className="mt-4 text-center">
                    <span className={`inline-block px-3 py-1 rounded-full text-sm ${
                      asta.startNow ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                    }`}>
